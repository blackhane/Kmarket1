...
public class ProductDAO extends DBCP {
	...
	//Ajax 리스트
	public List<ProductVO> ajaxProductList(String cate1, String cate2, String sort, int start) {
		List<ProductVO> product = new ArrayList<>();
		try {
			logger.info("상품목록 정렬 : " + sort);
			conn = getConnection();
			String sql = "SELECT * FROM `km_product` WHERE `cate1`=? AND `cate2`=? ORDER BY " + sort + " LIMIT ?,10";
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, cate1);
			psmt.setString(2, cate2);
			psmt.setInt(3, start);
			...
			while(rs.next()) {
				...
				vo.setReview(rs.getInt("review"));
				vo.setThumb1(rs.getString("thumb1"));
				vo.setDisPrice(vo.getPrice() - (vo.getPrice()/100*vo.getDiscount()));
				product.add(vo);
			}
			close();
		}catch(Exception e) {
	...
	//주문상품 찾기
	public CartVO selectOrderProduct(String prodNo, String count) {
		...
		try {
			...
			if(rs.next()) {
				...
				vo.setTotal((price-disPrice) * Integer.parseInt(count));
			}
			close();
		}catch(Exception e) {
			logger.error(e.getMessage());
