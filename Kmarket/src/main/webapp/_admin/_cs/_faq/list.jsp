...
<script>
	...
	$(function() {
		$(".group").val('${group}');
		if('${cate}' != ''){
			$('.cate').append('<option value="${cate}">${cate}</option>').show();
		}
		...
		if(a == '회원'){
			temp.show();
			temp.append('<option value="전체보기">전체보기</option>');
            ...
            <section id="admin-index">
                ...
                <form action="/Kmarket/admin/cs/faq/list.do" method="get">
	                <select name="group" class="group">
	                 ...
	                 <select name="cate" class="cate" style="display:none;"></select>
                    ...
                    <div class="admin_cs_list_div">
                        ...
                        <div class="page">
	                        <c:if test="${pageGroupStart gt 1}">
		                        <a href="/Kmarket/admin/cs/faq/list.do?group=${group}&cate=${cate}&pg=${pageGroupStart-1}" class="prev">
		                            <&nbsp;이전</a>
				        	</c:if>
	                        <c:forEach var="num" begin="${pageGroupStart}" end="${pageGroupEnd}">
				            	<a href="/Kmarket/admin/cs/faq/list.do?group=${group}&cate=${cate}&pg=${num}" class="num ${currentPage eq num ? 'current' : 'off' }">${num}</a>
				            </c:forEach>
		                    <c:if test="${pageGroupEnd lt lastPageNum}">
		                        	<a href="/Kmarket/admin/cs/faq/list.do?group=${group}&cate=${cate}&pg=${pageGroupEnd+1}" class="next">다음&nbsp;></a>
				            </c:if>
                        </div>
                    </div>
                </div>
